...
function CalcForecastExcelByDep() {
    ...
    //CalcForecastExcelByDepFinish(null)
    ...
    data.data().forEach(function (item) {
        ...
        var prev = (item.PrevValue != null ? item.PrevValue : 0);
        var fore = (item.Forecast != null ? item.Forecast : 0);
        if (prev == 0 && fore > 0) {
            collection.barcodes.push({ barcode: item.Barcode, sended: false, loaded: false })
        }
    });

    if (collection.barcodes.length > 0) {
        collection.barcodes[0].sended = true
        $.ajax({
            url: "https://s2010/complaints/api/info/GetForecastForecastCalculationItemParents?ForecastId=" + String(collection.forecastId) + "&Barcod=" + String(collection.barcodes[0].barcode),
            global: false,
            type: "GET",
            success: function (data) {
                if (typeof (data) == "object") {
                    CalcForecastExcelByDepGetParentNext(collection, collection.barcodes[0].barcode, data)
                };
            },
            error: function () {
                //alert("Произошла ошибка при формировании файла: ошибка получения состава")
                WaitHide("forecast_calc", "Произошла ошибка при формировании файла: ошибка получения состава. Баркод: " + String(item.barcode));
                document.body.style.cursor = 'default';
            }        
    });
    }
        ...
        WaitHide("forecast_calc", null);
        document.body.style.cursor = 'default';
        CalcForecastExcelByDepFinish(collection)
    }
}

function CalcForecastExcelByDepGetParentNext(collection, barcode, parents) {

    if (collection.finishedParents || collection.hasError)
        return;

    if (File == null)
        return;
    
    var allWithParetns = true;
    collection.barcodes.forEach(function (item) {
        if (item.barcode == barcode) {
            item.parents = parents;
            item.loaded = true;
        }
        if (!item.loaded)
            allWithParetns = false;
    })
    collection.barcodes.forEach(function (item) {
        if (!item.sended) {
            allWithParetns = false;
            item.sended = true
            $.ajax({
                url: "https://s2010/complaints/api/info/GetForecastForecastCalculationItemParents?ForecastId=" + String(collection.forecastId) + "&Barcod=" + String(item.barcode),
                global: false,
                type: "GET",
                success: function (data) {
                    if (typeof (data) == "object") {
                        CalcForecastExcelByDepGetParentNext(collection, item.barcode, data)
                        return;
                    };
                },
                error: function () {
    ...
    if (allWithParetns) {
        //WaitHide("forecast_calc", null);
        //document.body.style.cursor = 'default';
        collection.finishedParents = true;
        //// collections.barcodes[].parents[].Items[].Barcode /.Quan /.Quan1 /.RecipeNum

        var data = $("#grid_forecast").data("kendoGrid").dataSource
        var dataCalc = $("#grid_forecast_calc").data("kendoGrid").dataSource

        collection.barcodes.forEach(function (barcode) {
            barcode.parents.forEach(function (parent) {
                parent.Items.forEach(function (item, index, array) {
                    var exist = collection.calcs.filter(_calc => _calc.LocCode == item.Barcode && _calc.NumAkt == item.RecipeNum)
                    var Name = null;
                    var Rows = null;
                    // Фиктивная техкарта баркоа
                    if (item.RecipeNum == null && index == 0 && array.length > 1) {
                        data.data().forEach(function (itemName) {
                            if (itemName.Barcod == item.Barcode)
                                Name = itemName.Name;
                        });
                        Rows = [{ LocCode: array[1].Barcode }]
                    }
                    // Фиктивная техкарта проработки
                    if (item.RecipeNum === 0 && index == array.length - 2 && array.length > 1) {
                        Name = "Проработанный *" // Этого не надо
                        var NameIng = "";
                        dataCalc.data().forEach(function (itemName) {
                            if (itemName.Barcode == array[array.length - 1].Barcode)
                                NameIng = itemName.Name;
                        });
                        Rows = [{ LocCode: array[array.length - 1].Barcode, Name: NameIng }]
                    }
                    // 
                    if (exist.length == 0) {
                        if (item.Barcode == 840151) {
                            var ttt = 0;
                            ttt += 1;
                            ;
                        }
                        collection.calcs.push({ LocCode: item.Barcode, NumAkt: item.RecipeNum, Name: Name, Rows: Rows, Sended: Rows != null, Loaded: Rows != null })
                    }
                })
            })
        })
        ...
        if (collection.calcs.length > 0) {
            collection.calcs[0].Sended = true;
            $.ajax({
                url: "https://s2010/complaints/api/info/GetRecipe?CodGood=" + String(collection.calcs[0].LocCode) + "&NumAkt=" + String(collection.calcs[0].NumAkt),
                //url: "https://s2010/complaints/api/info/GetRecipe?CodGood=8411005&NumAkt=255000000	",
                global: false,
                type: "GET",
                success: function (data) {
                    if (typeof (data) == "object") {
                        CalcForecastExcelByDepGetCalcNext(collection, collection.calcs[0].LocCode, collection.calcs[0].NumAkt, data)
                        return;
                    };
                },
                error: function () {
                    ////alert("Произошла ошибка при формировании файла: ошибка получения состава")
                    //WaitHide("forecast_calc", "Произошла ошибка при формировании файла: ошибка получения техкарт. Баркод: " + String(item.LocCode) + "  Номер: " + String(item.NumAkt));
                    //document.body.style.cursor = 'default';
                    //collection.hasError = true;

                    // Такого рецепта нет
                    collection.calcsNotFound.push("Баркод: " + String(collection.calcs[0].LocCode) + "  Номер: " + String(collection.calcs[0].NumAkt))
                    CalcForecastExcelByDepGetCalcNext(collection, collection.calcs[0].LocCode, collection.calcs[0].NumAkt, null)
                    return;
                }
            })
    }
...
function CalcForecastExcelByDepGetCalcNext(collection, barcode, numakt, calc) {

    if (collection.finishedCalcs || collection.hasError)
        return;
    ...
    var allWithParetns = true;
    if (calc != null)
    collection.calcs.forEach(function (item) {
        if (item.LocCode == barcode) {
            if (item.Rows == null)
                item.Rows = calc.Rows;
            item.Name = calc.Name;
            item.Loaded = true;
        }
        if (item.NumAkt == null && item.Rows != null) {
            item.Rows.forEach(function (row) {
                if (row.LocCode == calc.LocCode && row.Name == null)
                    row.Name = calc.Name
            })
        }
        if (!item.Loaded)
    ...
    collection.calcs.forEach(function (item) {
        if (!item.Sended) {
            if (item.NumAkt != null) {
                ...
                item.Sended = true;
                $.ajax({
                    url: "https://s2010/complaints/api/info/GetRecipe?CodGood=" + String(item.LocCode) + "&NumAkt=" + String(item.NumAkt),
                    global: false,
                    type: "GET",
                    success: function (data) {
                        //setTimeout(() => {
                        if (typeof (data) == "object") {
                            CalcForecastExcelByDepGetCalcNext(collection, item.LocCode, item.NumAkt, data)
                        };
                        //}, 150)
                        i = collection.calcs.length;
                        return;
                    },
                    error: function () {
                        //setTimeout(() => {
                        ...
                        // Такого рецепта нет
                        var text = "Баркод: " + String(item.LocCode) + "  Номер: " + String(item.NumAkt);
                        if (collection.calcsNotFound.indexOf(text) == -1)
                            collection.calcsNotFound.push(text)
                        item.Loaded = true;
                        //}, 150)
                        i = collection.calcs.length;
                        return;
...
function CalcForecastExcelByDepFinish(collection) {
    //alert("Function in developing")

    if (collection != null && collection.calcsNotFound.length > 0) {
        alert("При получении отчета не были найдены следующие техкарты (существовавщие при расчете ингредиентов):\r\n" + collection.calcsNotFound.join("\r\n"))
    }
    ...
    //var data1 = $("#grid_forecast_calc");
    //var data2 = $("#grid_forecast_calc").data("kendoGrid");
    var data = $("#grid_forecast_calc").data("kendoGrid").dataSource;
    ...
    for (var iDep = 1; iDep <= 8; iDep++) {
        ...
        rows.push(
            {
                cells: [{ value: "", background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed },
        ...
        for (var iGr = 0; iGr < 5; iGr++) {
            ...
            data.data().forEach(function (item) {
                ...
                if (item.Barcode != null && item.Barcode != "" && String(item.Barcode).substring(0, 1) === String(iDep)) {
                    ...
                    switch (iGr) {
                        case 0: // New
                            needToAdd = prev == 0 && fore > 0;
                            digitColor = "#007700";
                            break;
                        case 1: // +
                            needToAdd = fore > prev && prev != 0;
                            digitColor = "#007700";
                            break;
                        case 2: // -
                            needToAdd = fore < prev && fore != 0;
                            digitColor = "#000099";
                            break;
                        case 3: // =
                            needToAdd = fore == prev;
                    ...
                    if (needToAdd) {
                        ...
                        var valAbs = fore - prev;
                        var valPer = Math.round((valAbs / prev) * 100);

                        var dividerAbs = valAbs < 1 ? (valAbs < 0.001 ? 1000 : 100000) : 100;
                        valAbs = (Math.round((valAbs) * dividerAbs) / dividerAbs);

                        valPer = (prev == fore ? ("0 %") :
                            ((valPer == Infinity) ? "+ 100" : ((fore > prev ? "+" : "") + String(valPer))) + " %");


                        prevFormat = prev == 0 ? "0" : (prev < 1 ? (prev < 0.001 ? "0.00000" : "0.000") : "0.00")
                        foreFormat = fore == 0 ? "0" : (fore < 1 ? (fore < 0.001 ? "0.00000" : "0.000") : "0.00")
                        ...
                        var newRow = {
                            cells: isFirst ? [
                                { value: groupNames[iGr], textAlign: "center", verticalAlign: "center", bold: true, wrap: true, borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed }
                            ] : []
                        }
                        ...
                        newRow.cells.push({ value: item.PrevValue, textAlign: "left", bold: (iGr == 0), format: prevFormat, borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin });
                        newRow.cells.push({ value: item.Forecast, textAlign: "left", bold: (iGr == 0), format: foreFormat, borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin });
                        ...
                        newRow.cells.push({ value: valPer, textAlign: "left", bold: (iGr == 0), color: digitColor, borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bMed });

                        rows.push(newRow);

                        //    {
                        //        cells: [
                        //            //{ value: (isFirst ? groupNames[iGr] : ""), textAlign: "center", verticalAlign: "center", bold: true },
                        //            ,
                        //            { value: item.Barcode, textAlign : "left", bold : (iGr == 0) },
                        //            { value: item.Name, textAlign : "left", bold : (iGr == 0) },
                        //            { value: item.Unit, textAlign : "left", bold : (iGr == 0) },
                        //            { value: item.PrevValue, textAlign: "left", bold: (iGr == 0), format: prevFormat },
                        //            { value: item.Forecast, textAlign: "left", bold: (iGr == 0), format: foreFormat },
                        //            { value: valAbs, textAlign : "left", bold : (iGr == 0), color : digitColor },
                        //            { value: valPer, textAlign : "left", bold : (iGr == 0), color : digitColor }
                        //        ]
                        //    })
    ...
    rows.push(
        {
            cells: [//{ value: "Прогноз", background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed },
            { value: "Баркод", background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed },
            { value: "Наименование", background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed }
            ]
        })
    rows.push({ cells: [{ value: null, textAlign: "left" }] })

    var newBarcodes = []
    collection.barcodes.forEach(function (item) {
        item.parents.forEach(function (parent) {
            if (parent.Items.length > 0) {
                var exist = newBarcodes.filter(_bc => _bc.Barcode == parent.Items[0].Barcode && _bc.RecipeNum == parent.Items[0].RecipeNum)
                if (exist.length == 0)
                    newBarcodes.push({ Barcode: parent.Items[0].Barcode, RecipeNum: parent.Items[0].RecipeNum })
            }
        })
    })

    newBarcodes.forEach(function (item) {
        //var settings = { nowLevel = 1 }
        var nowLevel = 0
        var history = [{ Barcode: item.Barcode, RecipeNum: item.RecipeNum }]
        FillForecastCalcExcelByDepNewGoods(item.Barcode, item.RecipeNum, collection, nowLevel, history, rows)
        rows.push({cells: [{ value: null, textAlign: "left" }]})
    })
    ...
    var sheetNewGoods =
    {
        columns: [//{ width: 50 },
            //{ width: 80 }, { width: 120 },
            { width: 65 }, { width: 65 }, { width: 65 }, { width: 65 },
            { width: 65 }, { width: 65 }, { width: 65 }, { width: 65 },
            { width: 65 }, { width: 65 }, { width: 65 }, { width: 65 },
            { width: 65 }, { width: 65 }, { width: 65 }, { width: 65 }],
        name: sheetNames[8],
    ...
    kendo.saveAs({
        ...
        //fileName: "ForecastCalcByDeps.xlsx"
    });

}


function FillForecastCalcExcelByDepNewGoods(barcode, numAkt, collection, nowLevel, history, rows) {
    var name = null;
    collection.calcs.forEach(function (item) {
        if (name == null) {
            if (item.LocCode == barcode)
    ...
    var newRow = {
        cells: [
            { value: barcode, textAlign: "left", bold: isBold },
            { value: String(name) + (numAkt != null ? ("    ТК №: "+String(numAkt)) : ""), textAlign: "left", bold: isBold }
        ]
    };
    for (var i = 0; i < nowLevel; i++)
        newRow.cells.unshift({ value: null });

    rows.push(newRow)

    var calc = collection.calcs.filter(_calc => _calc.LocCode == barcode && _calc.NumAkt == numAkt)
    if (calc.length != 0 && calc[0].Rows != null) {
        calc[0].Rows.forEach(function (row) {
            var akt = null;

            collection.barcodes.forEach(function (item) {
                item.parents.forEach(function (parent) {
                    if (parent.Items.length >= nowLevel + 1) {
                        var ok = true;
                        for (var i = 0; i <= nowLevel; i++)
                            if (!(parent.Items[i].RecipeNum == history[i].RecipeNum && parent.Items[i].Barcode == history[i].Barcode)) {
                                ok = false;
                            }
                        if (ok) {
                            ;
                        }
                        if (ok && parent.Items[nowLevel + 1].Barcode == row.LocCode)
                            akt = parent.Items[nowLevel + 1].RecipeNum;
                    }
                })
            })
            history[nowLevel + 1] = { Barcode: row.LocCode, RecipeNum: akt }
            FillForecastCalcExcelByDepNewGoods(row.LocCode, akt, collection, nowLevel + 1, history, rows)
        })
    }
...
function AppendItem() {
    ...
    $.ajax({
        url: "https://" + host + complaints + "/api/Info/GetBarcodeName?CodGood=" + String(res),
        type: "GET",
        success: function (dataResp) {
            if (dataResp == "Баркод не найден") {
                alert(dataResp)
            }
            else if (String(dataResp).length > 500)
            {
                // Вылетает авторизация AAAAAAAAAAAAAA
                alert("Ошибка соединения")
            }
            else {
                AppendItemProcess(res, dataResp)
            }
        },
        error: function () {
