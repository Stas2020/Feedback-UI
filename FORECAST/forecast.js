        ...
        success: function (data) {
            if (typeof (data) == "object") {
                if (data.Id == -1) {
                    ...
                    data.FuncList.forEach(function (func) {

                        func.Role;

                        var Function            = new Object();
                            Function.FunctionId = func.Function.FunctionId;

                        userFunctionId = func.Function.FunctionId;


                        var Permissions        = new Object();
                            Permissions.Edit   = func.NativeRight.Edit;
                            Permissions.Delete = func.NativeRight.Delete;
                            Permissions.Add    = func.NativeRight.Add;
                            Permissions.View   = func.NativeRight.View;

                        Function.Permissions = Permissions;

                        RoleUser.AvailableFunction[func.Function.FunctionName] = Function;

                    });

                    if (RoleUser.UserId == 2148) {
                        // Школа поваров не может выбирать фильтры, подразделения и ДИМы
                        $("#forecast_filters_bar").css('visibility','hidden')
                        // Также у них свой формат excel-файла прогноза
                        $("#calcForecastExcelByDep").attr("onClick", "ChefSchoolCalcExcel()")
                    }
...
function RefreshForecast() {
    ...
    $.ajax({
        url    : "https://" + host + "/complaints/api/info/GetForecastTemplate?" + filtersGet,
        global : false,
        type   : "GET",
        success: function (data) {
            if (typeof (data) == "object") {
...
function PrepareDataForecast(data, openedFile, openedDate, dontsavefile) {
    ...
    if (openedFile) {
        ...
        data.Table.forEach(function (item) {
            ...
            //Fictive oper
            if (item.SrcM == null && item.SrcI == null && item.SrcO == null && item.SrcC == null) bc["Oper"].push(GetCompactScr({
                num_doc  : null,
                price    : null,
                shops    : [88],
                src      : "M",
        ...
        toolbar: ["excel",
            {
                name    : "add",
                text    : "Удалить",
                template: '<a class="k-button" href="\\#" onclick="AppendItem()">➕ Добавить баркод</a>'
            },
            {
                name    : "del",
                text    : "Удалить",
            ...
            {
                name    : "filterIOOn",
                text    : "Показать только ввод/вывод",
                template: '<a class="k-button" href="\\#" onclick="FilterIOSwitch(true)">🕶 Показать только ввод/вывод</a>'
            },
            {
                name    : "filterIOOff",
                text    : "Показать все",
                template: '<a class="k-button" href="\\#" onclick="FilterIOSwitch(false)">👓 Показать все</a>'
            }
        ],
...
function FilterIOSwitch(hide)
{
	...
	for (var i = 0; i < items.length; i++) {
	    {
        	var $row    = $('#grid_forecast').find("[data-uid='"+items[i].uid+"']");  // find grid row by uid
        	var visible = !hide;
		if(!hide)
	        	$row.show();
		else
		{
			var res     = GetOperDesc(items[i].Barcod, items[i].Oper)
			    visible = items[i].Barcod == null || (res.text != null && (res.text.indexOf("Ввод") != -1 || res.text.indexOf("Вывод") != -1))
		 	if(visible)
				$row.show();
			else
		        	$row.hide();
		...
		if(visible)
		{
			$('tr[data-uid="' + items[i].uid + '"] td:nth-child(n)').css("background-color", even?"#fff":"#f1f1f1")
			even = !even;
		}
	    }
	}
...
function CalcForecastCalc() {
    ...
    $.ajax({
        url        : "https://"+ host + "/complaints/api/info/GetForecastCalculation",
        global     : false,
        type       : "POST",
        contentType: "application/json",
        data       : JSON.stringify(send_data),
...
function PrepareDataForecastCalc(data) {
    ...
    function excelexport(e) {
            ...
            row0.cells[7].value            = "";
            row0.cells[7].background       = "#ffffff"

            //
            for (var rowIndex = 1/*0*/; rowIndex < sheet.rows.length; rowIndex++) {
                var row = sheet.rows[rowIndex];

                row.cells[0].textAlign = "left"
                row.cells[1].textAlign = "left"
                row.cells[2].textAlign = "left"
                row.cells[3].textAlign = "left"
                row.cells[4].textAlign = "left"
                row.cells[5].textAlign = "left"
                row.cells[6].textAlign = "left"
                row.cells[7].textAlign = "left"

                var prev = row.cells[0].value;
                var fore = row.cells[1].value;

                var valAbs = fore - prev;
                var val    = Math.round(((fore - prev) / prev) * 100);

                row.cells[3].value = valAbs;
                row.cells[4].value = (prev == fore ? ("0 %") :
                    ((val == Infinity) ? "+ ?" : ((fore > prev ? "+" : "") + String(val))) + " %");


                row.cells[0].format = prev == 0 ? "0" : (prev < 1 ? "0.000" : "0.00")
                row.cells[1].format = fore == 0 ? "0" : (fore < 1 ? "0.000" : "0.00")

                if (prev != fore) {
                    row.cells[3].format = valAbs < 1 ? "0.000" : "0.00"
                    var       color     = (fore > prev) ? "#007700" : "#000099";
                    row.cells[3].color  = color;
                    row.cells[4].color  = color;
                }
            }
    ...
    $("#grid_forecast_calc").kendoGrid({
        toolbar: [/*"excel",*/
            {
                name    : "filterCalcChOn",
                text    : "Показать только ввод/вывод",
                template: '<a class="k-button" href="\\#" onclick="FilterCalcChSwitch(true)">🕶 Показать только измененные</a>'
            },
            {
                name    : "filterCalcChOff",
                text    : "Показать все",
                template: '<a class="k-button" href="\\#" onclick="FilterCalcChSwitch(false)">👓 Показать все</a>'
            }],
        excelExport: excelexport,
        excel      : {
            allPages: true,
        ...
        columns: [{
            ...
            },
            {
                ...
                width   : "105px",
                template: function (dataItem) {
                    var prev    = (dataItem.PrevValue != null ? dataItem.PrevValue : 0);
                    var fore    = (dataItem.Forecast != null ? dataItem.Forecast : 0);
                    var unit    = dataItem.Unit;
                    var val     = fore-prev;
                    var divider = val < 1 ? 1000 : 100;
                        val     = (Math.round((val) * divider) / divider);
                    return (prev == fore ? ("0&nbsp;"+unit) : ("<b style='" + ((fore > prev) ? 'color:green' : 'color:blue') + "'>"
                        + ((fore > prev ? "+" : "") + String(val))) + "&nbsp;" + unit+"</b>");
                }
            },
            {
                field   : "Change",
                ...
                template: function (dataItem) {
                    ...
                    var fore   = (dataItem.Forecast != null ? dataItem.Forecast : 0);
                    var valAbs = fore-prev;
                    var val    = Math.round((valAbs / prev) * 100);
...
function FilterCalcChSwitch(hide)
{
        document.body.style.cursor = 'wait';
    var grid                       = $('#grid_forecast_calc').data('kendoGrid');
    var items                      = grid.dataSource.view();
	...
	for (var i = 0; i < items.length; i++) {
	    {
        	var $row    = $('#grid_forecast_calc').find("[data-uid='"+items[i].uid+"']");  // find grid row by uid
        	var visible = !hide;
		if(!hide)
	        	$row.show();
		else
		{
			visible = items[i].PrevValue != items[i].Forecast
		 	if(visible)
				$row.show();
			else
		        	$row.hide();
...
function detailInit(e) {
    ...
    if (e.data.Parents == null) {
        $.ajax({
            url    : "https://" + host + "/complaints/api/info/GetForecastForecastCalculationItemParents?ForecastId=" + String(File.id) + "&Barcod=" + String(e.data.Barcode),
            global : false,
            type   : "GET",
            success: function (data) {
                if (typeof (data) == "object") {
    ...
    e.data.Parents.forEach(function (item, idx) {
        ...
        item.Items.forEach(function (itm, idx, array) {
            ...
            par.push(
                ...
                + ((idx == 0) ? (item.IsNew ? " Прогноз: " : " Продано: ") : (outString + " Кол-во: "))
...
function FileOpen() {
    ...
    $.ajax({
        url    : "https://" + host + "/complaints/api/info/GetForecastFiles?OnlyNames=false",
        global : false,
        type   : "GET",
        success: function (data) {
            if (typeof (data) == "object") {
...
function FileOpenOkClick() {
    ...
    if (selectedItem != null) {
        ...
        $.ajax({
            url        : "https://" + host + "/complaints/api/info/GetForecast?Id=" + String(selectedItem.Id),
            global     : false,
            type       : "POST",
            contentType: "application/json",
            data       : JSON.stringify(send_data),
...
function FileOpenPrepareData(data) {
    ...
    PrepareDataForecast(data.Forecast, true, null, true);//!!!!!!!
    ...
    File.prevMonthStr = months[prevMonth];
    File.foreMonthStr = months[monthMinus.getMonth()];  //data.ForecastMonth;
    SetFileDescriptions(data);
}

function CheckFreeFileName(fileName, saveAs, calcAfterSave) {
    $.ajax({
        url    : "https://" + host + "/complaints/api/info/GetForecastFiles?Count=-1&OnlyNames=true",
        global : false,
        type   : "GET",
        success: function (data) {
            if (typeof (data) == "object") {
...
function FileSaveSend(id, name, calcAfterSave, saveAs) {
    ...
    send_data.Forecast.DeletedBarcodes = 
        ($("#forecast_deleted").val() != "") ? 
            $("#forecast_deleted").val().split(",") : 
            [];
...
function FiltersApply() {
    ...
    $.ajax({
        url        : "https://" + host + "/complaints/api/info/GetForecast?Id=" + String(fileId),
        global     : false,
        contentType: "application/json",
        type       : "POST",
        data       : JSON.stringify(send_data),
...
function FileDelClick() {
    ...
    if (selectedItem != null) {      
        ...
        $.ajax({
            url        : "https://" + host + "/complaints/api/info/PostForecastDelete",
            global     : false,
            type       : "POST",
            contentType: "application/json",
            data       : JSON.stringify(send_data),
...
function ForeCastCGSSelectFromDims() {
    ...
    if (selectedDims.length == 0) {
        alert("ДиМы не выбраны")
        return;
    }

    var dims = $("#forecast_dim_mode").val();
    $.ajax({
        url    : "https://" + host + complaints + "/api/Info/GetDimCassGroups?Dims=" + dims,
        global : false,
        type   : "GET",
        success: function (data) {
            if (typeof (data) == "object") {
                if (data.length > 0) {
                    var dataGrid = $("#grid_forecast_cass_groups").data("kendoGrid").dataSource;
                    dataGrid.data().forEach(function (item) {
                        item.Sel = data.indexOf(item.Num) != -1
                    });
                    $("#grid_forecast_cass_groups").data("kendoGrid").setDataSource(dataGrid);

                    $("#window_forecast_cass_groups").data("kendoWindow").center().close();
                }
                else
                    alert("Не найдено кассовых групп")
            } else {
                onExit();
            }
        }
...
// Возвращает строчку с нулём с количеством цифр после запятой, соответствющей точности числа
function FormattedZeroString(number) {
    if (number == 0)
        return "0";
    if (number >= 1)
        return "0.00";
...
// Возвращвет функцию, которая убирает анимацию ожидания и выводит сообщение об ошибке с заданным баркодом
function createAjaxErrorHandler(barcode) {
    return () => {
        WaitHide("forecast_calc", "Произошла ошибка при формировании файла: ошибка получения состава. Баркод: " + String(barcode));
        document.body.style.cursor = 'default';
    }
}
// 
function getNewProductParentsAjax(product, forecastId, recipes) {
    function updateParentRecipeWithNewIngredient(parent) {
        return function() {
            var label = parent.nested === "" ? "" : "в составе п/ф " + parent.nested;
            var recipe = recipes[parent.barcode] 
            recipe[product.barcode] = { name: product.name, isNew: true, label: label }
            console.log("update " + recipe.name + " with " + product.name)
        }
    }

    return $.ajax({
        url    : "https://" + host + "/complaints/api/info/GetIngredientTopLevelSources?ForecastId=" + String(forecastId) + "&Barcod=" + String(product.barcode),
        global : false,
        type   : "GET",
        success: function (productParents) {
            if (typeof (productParents) == "object") {
                productParents.forEach(function(parent) {
                    if (parent.barcode in recipes.meta.promiseChains) {
                        var chain = recipes.meta.promiseChains[parent.barcode]
                        chain.then(updateParentRecipeWithNewIngredient(parent))
                        return;
                    }
                    var getRecipe = getAndCreateRecipeAjax(parent, recipes)
                    recipes.meta.promiseChains[parent.barcode] = getRecipe.then(function() {
                        console.log("created " + recipes[parent.barcode].name  + " by " + product.name)
                    }).then(updateParentRecipeWithNewIngredient(parent))
                    
                })       
            } 
        },
        error: createAjaxErrorHandler(product.barcode)   
    })
}

function getAndCreateRecipeAjax(product, recipes) {
    return $.ajax({
        url: "https://" + host + "/complaints/api/info/GetRecipe?CodGood=" + String(product.barcode) + "&NumAkt=" + String(product.recipeNum),
        global : false,
        type   : "GET",
        success: function (recipeData) {
            if (typeof (recipeData) == "object") {
                var recipe = { name: recipeData.Name }
                recipeData.Rows.forEach((ingredient) => 
                    recipe[ingredient.LocCode] = { name: ingredient.Name, isNew: false, label: '' }
                );
                recipes[product.barcode] = recipe;
            };
        },
        error: function () {
            recipes.meta.notFound.push("Баркод: " + String(product.barcode) + "  Номер: " + String(product.recipeNum))
        },
    })
}


function getDimTable() {
    return $.ajax({
        url    : "https://" + host + complaints + "/api/Info/GetDimTable?ForecastId=" + File.id,
        type   : "GET",
        success: function(data) {
            return data;   
        },
        error: function () {
            alert("Не удалось получить список ДИМов")
        }
    })
}
...
function getName(barcode) {
    return $.ajax({
        url    : "https://" + host + complaints + "/api/Info/GetBarcodeName?CodGood=" + barcode,
        type   : "GET",
        success: function (name) { return name; },
        error: function () {
            alert("Неверный баркод")
        }
    });
}


var departmentName = (departmenNumber) => DepartmentList.find(department => department.DepNum == departmenNumber).DepName;


var boldCell  = (value) => { return { value: value, bold: true}; }
var cell      = (value) => { return { value: value }; }
var emptyCell = ()      => { return { value: null}; }

var globalDim

function createAndPushDimSheet(dim, sheets) {
    globalDim = dim
    if (!dim.Ok) {
        var errorMsg = "Дим " + dim.Id + '.' + dim.Version + ': ' + dim.Message
        alert(errorMsg);
        return Promise.resolve();
    }

    function createDimRow(product) {
        var cells = [
            {value: product.Barcode, textAlign: "left"},
            {value: product.Name,    textAlign: "left"},
            {value: product.Umtype,  textAlign: "left"},
            {value: product.Ckassa,  textAlign: "left"},
        ];
        for (let depNum in dim.Departments) {
            var value;
            if (product.DepList.includes(depNum)) {
                value = depNum
            } else {
                value = ""
            }
            console.log("" + product.Name + " " + depNum + ":" + value)
            cells.push({ value: value, textAlign: "left" })
        }
        return {cells: cells}
    }

    return Promise.resolve().then(function() {
        var fourEmptyCells = [ emptyCell(), emptyCell(), emptyCell(), emptyCell()]
        var rows = [
            { cells: [
                boldCell("ДИМ " + dim.Id + '.' + dim.Version + ": " + dim.Name + " Дата внедрения " + dim.EntryDate)
            ]},
            { cells: [
                emptyCell(),
                boldCell('Версия ' + dim.Version),
                boldCell('Операция'),
                emptyCell(),
                ...Object.keys(dim.Departments).map(boldCell)
            ]},
            { cells: [
                ...fourEmptyCells, 
                ...Object.values(dim.Departments).map(boldCell)
            ]}
        
        ];
        for (let product of dim.Rows) {
            rows.push(createDimRow(product))
        }
        sheets.push({
            name: 'ДИМ ' + dim.Id + '.' + dim.Version, 
            rows: rows, 
        });
    })
}

function CalcForecastExcelByDep() {
    if (File == null) {
    ...
    var dataSource = $("#grid_forecast_calc").data("kendoGrid").dataSource;
    var newIngredients = [];
    
    dataSource.data().forEach(function (item) {
        var prev = item.PrevValue ?? 0;
        var fc   = item.Forecast ?? 0;
        if (prev == 0 && fc > 0) {
            newIngredients.push({ name: item.Name, barcode: item.Barcode})
        }
    });
    ...
    newIngredients.forEach((product) => getParentsCalls.push(getNewProductParentsAjax(product, File.id, recipes)))
    $.when(...getParentsCalls).done(function() {
        console.log(recipes)
        Promise.all(Object.values(recipes.meta.promiseChains)).then(function() {
            CalcForecastExcelByDepFinish(recipes)
        })
    })
...
function CalcForecastExcelByDepFinish(recipes) {

    WaitHide("forecast_calc", null);
    document.body.style.cursor = 'default';

    if (recipes != null && recipes.meta.notFound.length > 0) {
        alert(
            "При получении отчета не были найдены следующие техкарты (существовавщие при расчете ингредиентов):\r\n" 
            + recipes.meta.notFound.join("\r\n"))
    }
    ...
    var sheetNames = [
        ...
        "3. Молочно - Маслян и Яичная гастрономия",
        "4. Рыбная гастрономия",
        "5. Напитки, Табак",
        "6. Овощи, Фрукты, Ягоды, Грибы, Орехи",
        "7. Хлебобулочные и Кондитерские Изделия",
        "8. Производимые товары",
        "9. Товары с новыми ингредиентами",
        "10. ДИМ"
    ];
    ...
    var bMed  = { color: "#000000", size: 2 };
    var bThin = { color: "#000000", size: 1 };
    
    
    
    var dataSource = $("#grid_forecast_calc").data("kendoGrid").dataSource;
    
    for (var iDep = 1; iDep <= 8; iDep++) {
        var rows        = []
        var mergedCells = [];
        rows.push(
            {
                cells: [
                    { value: "",                                background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed },
                    { value: "Баркод",                          background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed },
                    { value: "Наименование",                    background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed },
                    { value: "Ед. изм.",                        background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed },
                    { value: "Расход за " + File.prevMonthStr,  background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed },
                    { value: "Расход за " + File.foreMonthStr,  background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed },
                    { value: "Изменение объёма",                background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed },
                    { value: "Изменение объёма в %",            background: "#BBBBBB", textAlign: "left", borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed }
                ]
            })

            for (var iGr = 0; iGr < 5; iGr++) {
                
                var isFirst = true;
                dataSource.data().forEach(function (item) 
                {
                if (item.Barcode != null && item.Barcode != "" && String(item.Barcode).substring(0, 1) === String(iDep)) 
                {
                    var needToAdd = false;
                    ...
                    switch (iGr) {
                        case 0:   // New
                            needToAdd  = prev == 0 && fc > 0;
                            digitColor = "#007700";
                            break;
                        case 1:   // +
                            needToAdd  = fc > prev && prev != 0;
                            digitColor = "#007700";
                            break;
                        case 2:   // -
                            needToAdd  = fc < prev && fc != 0;
                            digitColor = "#000099";
                            break;
                        case 3:   // =
                            needToAdd  = fc == prev;
                    ...
                    if (needToAdd) {
                        ...
                        var valAbs = fc - prev;
                        var valPrv = Math.round((valAbs / prev) * 100);

                        var dividerAbs = valAbs < 1 
                            ? (valAbs < 0.001 ? 1000 : 100000) : 100;
                            
                        valAbs = (Math.round((valAbs) * dividerAbs) / dividerAbs);
                        ...
                        if (isFirst) {
                            newRow.cells.push({ value: groupNames[iGr], textAlign: "center", verticalAlign: "center", bold: true, wrap: true, borderTop: bMed, borderBottom: bMed, borderLeft: bMed, borderRight: bMed })
                        }
                        newRow.cells.push({ value: item.Barcode,   textAlign: "left", bold: (iGr == 0),                                         borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin });
                        newRow.cells.push({ value: item.Name,      textAlign: "left", bold: (iGr == 0),                                         borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin });
                        newRow.cells.push({ value: item.Unit,      textAlign: "left", bold: (iGr == 0),                                         borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin });
                        newRow.cells.push({ value: item.PrevValue, textAlign: "left", bold: (iGr == 0), format: FormattedZeroString(prev),      borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin });
                        newRow.cells.push({ value: item.Forecast,  textAlign: "left", bold: (iGr == 0), format: FormattedZeroString(fc),        borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin });
                        newRow.cells.push({ value: valAbs,         textAlign: "left", bold: (iGr == 0), color: digitColor,                      borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin });
                        newRow.cells.push({ value: valPrv,         textAlign: "left", bold: (iGr == 0), color: digitColor,                      borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bMed  });
            ...
            if (!isFirst)
            {
                ...
                var span = 1;
                while (!isNaN(rows[iRow].cells[0].value) && iRow > 0) 
                {
                    span++;
                    iRow--;
                }
                if (iRow > 0) 
                {
                    for (var iCell = 0; iCell < rows[rows.length - 1].cells.length; iCell++)
                        rows[rows.length - 1].cells[iCell].borderBottom = bMed;

                    if (span > 1) {
                        rows[iRow].cells[0].rowSpan = span;
                        if (span > rows[iRow].cells[0].value.length) {

                            //var wrapped = "";
                            //for (var iS = 0; iS < rows[iRow].cells[0].value.length; iS++)
                            //    wrapped += rows[iRow].cells[0].value[iS] + ((iS < rows[iRow].cells[0].value.length - 1) ? "\r\n" : "");

                            //rows[iRow].cells[0].value = wrapped;
                            //;
                        }
                    }
                    //mergedCells.push("A" + String(iRow + 1) + ":A" + String(iRow + 1 + span));
                }
            }
        ...
        var sheetDep = 
        {
            ...
            columns    : [{ width: 100 }, { width: 130 }, { width: 430 },  { width: 120 }, { width: 140 }, { width: 140 }, { width: 190 }, { width: 190 }],
            name       : sheetNames[iDep-1],
            rows       : rows
        }
        sheets.push(sheetDep);
    }
    ...
    rows.push(
        {
            cells: [     
                { value: "Баркод",                   background: "#BFBFBF", textAlign: "left", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
                { value: "Наименование",             background: "#BFBFBF", textAlign: "left", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
                { value: "Баркод ингредиента",       background: "#BFBFBF", textAlign: "left", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
                { value: "Наименование ингредиента", background: "#BFBFBF", textAlign: "left", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
                { value: "Новый ингредиент",         background: "#BFBFBF", textAlign: "left", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
                { value: "В составе п/ф",            background: "#BFBFBF", textAlign: "left", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
            ]
        }) 

    for (var barcode in recipes) {
        if (barcode != 'meta') {
            var newRows = FillForecastCalcExcelByDepNewGoods(barcode, recipes[barcode])
            rows.push(...newRows);
            var emptyRowCells = []
            for (let i = 0; i < rows[0].cells.length; i++) {
                emptyRowCells.push({ value: null, background: "#BFBFBF", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin})
            }
            rows.push({cells: emptyRowCells})
        }
    }
    var finishingBottomBorderRow = []
    for (let i = 0; i < rows[0].cells.length; i++) {
        finishingBottomBorderRow.push({ value: null, borderTop: bThin })
    }
    rows.push({cells: finishingBottomBorderRow})


    var sheetNewGoods = 
    {
        columns: [ { width: 75 }, { width: 350 }, { width: 130 }, { width: 400 }, { width: 130 }, { width: 150 } ],
        name   : sheetNames[8],
        rows   : rows
    }
    sheets.push(sheetNewGoods);
    console.log(sheets)

    getDimTable().then(function(dimList) {
        createDimTableCalls = []
        dimList.forEach(dim => {
            createDimTableCalls.push(createAndPushDimSheet(dim, sheets))
        })
        return Promise.all(createDimTableCalls)
    })
    .then(function() {
        console.log(sheets);
        ...
        kendo.saveAs({
            ...
            fileName: "Ингредиенты.xlsx"
        });
    })  
}


function FillForecastCalcExcelByDepNewGoods(barcode, recipe) {
    var red = "#FF0000"
    var bThin = { color: "#000000", size: 1 };
    var newIngredientString = "Новый ингредиент"
    var rows = []
    rows.push({
        cells: [
            { value: barcode,     textAlign: "left", bold: true, borderTop: bThin, borderBottom: bThin, borderLeft: bThin,  borderRight: bThin },
            { value: recipe.name, textAlign: "left", bold: true, borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
            { value: null,        borderTop: bThin },
            { value: null,        borderTop: bThin },
            { value: null,        borderTop: bThin },
            { value: null,        borderTop: bThin, borderRight: bThin},
        ]
    });

    for (var ingredientBarcode in recipe) {
        if (ingredientBarcode != 'name') {
            var ingredient = recipe[ingredientBarcode];
            var newIngredientLabel = ingredient.isNew ? newIngredientString : "";
            var newRow = {
                cells: [
                    { value: null,                                                                      borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin},
                    { value: null,                                                                      borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
                    { value: ingredientBarcode,  textAlign: "left", bold: ingredient.isNew,             borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
                    { value: ingredient.name,    textAlign: "left", bold: ingredient.isNew,             borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin},
                    { value: newIngredientLabel, textAlign: "left", bold: ingredient.isNew, color: red, borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin},
                    { value: ingredient.label,   textAlign: "left", bold: ingredient.isNew,             borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin}
                ]
            }
            rows.push(newRow);
        }
...
function ChefSchoolCalcExcel() {
    ...
    var dataSource  = $("#grid_forecast_calc").data("kendoGrid").dataSource; 
    var ingredients = dataSource.data();

    for (var ingredient of ingredients) {
        var forecastValue = ingredient.Forecast ?? 0;
        var newRow = {
            cells: [
                { value: ingredient.Barcode, textAlign: "left", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
                { value: ingredient.Name,    textAlign: "left", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
                { value: ingredient.Unit,    textAlign: "left", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin },
                { value: forecastValue,      textAlign: "left", borderTop: bThin, borderBottom: bThin, borderLeft: bThin, borderRight: bThin, format: FormattedZeroString(forecastValue) }
            ]
        }
        rows.push(newRow)
    }
    ...
    var sheetDep = 
    {
        columns    : [{ width: 130 }, { width: 430 },  { width: 120 }, { width: 140 }],
        name       : "Заказ",
        rows       : rows
    }
    sheets = [sheetDep, ];
...
function AppendItem() {
    ...
    if (File == null || (File != null && File.createdMenu && !File.savedMenu)) {
        alert("Для добавления позиции необходимо сохранить прогноз")
        return;
    }

    var res = prompt("Введите баркод");
    if (res === null)
        return;
  
    getName(String(res)).done(function(name) {
        if (name == "Баркод не найден") {
            alert(name)
        }
        else if (String(name).length > 500)
        {
            // Вылетает авторизация AAAAAAAAAAAAAA
            alert("Ошибка соединения")
        }
        else {
            AppendItemProcess(res, name)
        }
    })

}

function AppendItemProcess(barcode, name) {
    var data          = $("#grid_forecast").data("kendoGrid").dataSource;
    var newDataSource = [];
