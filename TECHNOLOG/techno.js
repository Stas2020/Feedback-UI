...
let usersBTK = ["manager", "r.m.valladares@coffeemania.ru", "t.andreeva@coffeemania.ru", "n.yangildina@coffeemania.ru", "techno"];
        ...
        success: function (data) {
            if (typeof (data) == "object") {
                ...
                if (UserDepLinks.length > 0) {
                    ...
                    deps.unshift({ value: 70, text: "70 Учебный центр" });
        ...
        { 
            ...
            title: "БТК",
                ...
                template: function (dataItem) {
                    if (String(dataItem.NumAkt).startsWith("88") || String(dataItem.NumAkt).startsWith("70")){
...
function PrepareCalsData(data, author) 
{
    ...
    if (!currentRecipe.Recipe){
        ...
        currentRecipe.Recipe.LocCode = data.LocCode;
        currentRecipe.Recipe.Rows = data_source;
    }
    //console.log(currentRecipe);
...
function getInfoBTK(rowIndex = -1) {
    ...
    source.dataSource.data().forEach(function (item, index){
        if (rowIndex == -1 || rowIndex == index){
            if (String(item.NumAkt).startsWith("88") || String(item.NumAkt).startsWith("70")){
                $.ajax({
...
function openWindowBTK(index){
    ...
    currentRecipe = {
        Index: index,
        RecipeChange: data,
        CurrentBarcode: data.LocCode,
        CurrentName: data.Name,
        BaseBarcode: data.LocCode,
    };
    ...
    $.ajax({
        url: "https://" + host + "/complaints/api/igestori/GetExtraBarcodes?barcode=" + data.LocCode,
        type: "GET",
        success: function (dataIn) {
            dataIn = dataIn.map( (itm) => {
                return {
                    title: itm.bar_cod + " " + itm.name_bar,
                    Barcode: itm.bar_cod,
                    Name: itm.name_bar
                };
            })
            $("#win_btk_barcode").data("kendoDropDownList").setDataSource(dataIn);
            $("#win_btk_barcode").data("kendoDropDownList").select(function(dataItem) {
                return Number(dataItem.Barcode) == Number(data.LocCode);
            });
        }
    });
...
function getListTechcard(message = "") {
    ...
    $.ajax({
        ...
        url: "https://" + host + "/complaints/api/igestori/GetListTechcard?barcode=" + currentRecipe.CurrentBarcode,
		type: "GET",
		success: function (data) {
            //console.log(data);
            if (typeof (data) == "object") {
...
function sendTechcard(uuid){    
    ...
    let title = currentRecipe.CurrentName; //recipe.Name;
    let barcode = currentRecipe.CurrentBarcode; //recipe.LocCode;
...
function kengoObjectsInit(){
    ...
    $("#win_btk_barcode").kendoDropDownList({
        dataTextField: "title", dataValueField: "Barcode", placeholder: "", clearButton: false, 
        change: function(event){
            //обработка изменения баркода блюда (+ название)
            currentRecipe.CurrentBarcode = this.value();
            let itm = this.dataSource.data().find((e) => e.Barcode == currentRecipe.CurrentBarcode);
            if (itm) {
                currentRecipe.CurrentName = itm.Name;
            }            
            getListTechcard();
        }
    });    
...
function isBTKUser() {
    return usersBTK.includes(RoleUser.Login.toLowerCase());
