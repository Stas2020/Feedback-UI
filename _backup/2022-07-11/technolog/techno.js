...
function PrepareData(data) {
    ...
    $("#grid_recipe_changes").kendoGrid({
        ...
        columns: [{
            hidden: (RoleUser.Login != "manager" && RoleUser.Login != "r.m.valladares@coffeemania.ru"),
            field: "btk",
            title: "БТК",
                width: "60px",
                    ...
                    success: function (data) {
                        ...
                        if (typeof (data) == "object") {
                            if (data.length > 0){                            
                                ...
                                if (diff > 0) {
                                    let title = "Дата изменения техкарты больше даты изменения БТК на " + diff + " " + getNoun(diff, "день", "дня", "дней") + "!";
...
function exportToPDF(sender, param){    
    ...
    .done(function (data) {        
        ...
        .done(function (data) {
            $("#btk_print_form").css("display","none");
            FilePDFFree = data;        
            if (sender){
...
function fillPrintForm(priceFree = false){    
    ...
    let totalPrice = data_source.reduce((previousValue, currentValue) => previousValue + currentValue.Total, 0);
