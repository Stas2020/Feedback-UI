...
function getSanPinDetails(depId, dateStart = "", dateEnd = "", checkListId = 0) {
    ...
    try {
        ...
        var SD = (dateStart == "" ? $("#datepickerstart_pah").val().split('/') : dateStart.split('/'));
        var ED = (dateEnd == "" ? $("#datepickerend_pah").val().split('/') : dateEnd.split('/'));
...
// *******************************************************************************************************************************************
// ********* доработки для вкладки "Таблица Качества"

//инициализация объектов kendo 
function kengoObjectsInit(){
    $("#quality_month").kendoDropDownList({
        dataTextField: "title", 
        dataValueField: "value", 
    ...
    $("#quality_month").data("kendoDropDownList").select(function(dataItem) {
        return dataItem.value == new Date().getMonth();
    });
    ...
    $("#quality_year").kendoDropDownList(
        {placeholder: "", 
        clearButton: false,
        dataSource: yearsList,
    });
    $("#quality_year").data("kendoDropDownList").select(function(dataItem) {
        return dataItem.toString() == "2022";
    });
}

// обновление отчета "Таблица Качества"
function RefreshQuality(){

    var grid = $("#grid_quality").data("kendoGrid");
    if (grid) {
        grid.destroy();
    }
    $("#grid_quality").children().remove();

    $("#refreshQuality").attr('disabled', true);
    $("#refreshQuality").attr("title", "Выполняется запрос, дождитесь его завершения!");

    WaitShow("quality");
    let monthDate = $("#quality_year").data("kendoDropDownList").value() + $("#quality_month").data("kendoDropDownList").value().padStart(2,"0") + "01";
    console.log(monthDate);
    //let codeShops = $("#quality_departments").val();
    //console.log(codeShops);

    $.ajax({
        url: "https://" + host + "/complaints/api/info/GetQualityTable?MonthDate=" + monthDate,
        global: false,
        type: "GET",
        success: function (data) {
            console.log(data.ReportResult);
            WaitHide("quality");
            $("#refreshQuality").attr('disabled', false);
            $("#refreshQuality").attr("title", "");
            if (typeof (data) == "object") {
                PrepareQualityData(data.ReportResult)
        ...
        error: function (error) {
            ...
            $("#refreshQuality").attr('disabled', false);
            $("#refreshQuality").attr("title", "");
            WaitHide("quality", "Произошла ошибка при загрузке отчета!");
        }
    });
}

function PrepareQualityData(data) {

    //исправляем наименования подразделений
    data.forEach(function (item) {
        let dep = dep_list.filter(_dep => _dep.DepNum == item.DepNum)[0];
        item.DepName = dep ? dep.DepName : item.DepName;    
    });
    data.sort(function (a, b) { return ('' + a.DepName).localeCompare(b.DepName); });
    data.push({
        DepNum: 0,
        DepName: "ИТОГО",
        NegativeFeedbackCount: data.reduce((previousValue, currentValue) => previousValue + currentValue.NegativeFeedbackCount, 0),
        StaffDismissed: data.reduce((previousValue, currentValue) => previousValue + currentValue.StaffDismissed, 0),
        StaffTurnover: (data.reduce((previousValue, currentValue) => previousValue + currentValue.StaffTurnover, 0) / data.length).toFixed(2),
        PoisoningComplaint: data.reduce((previousValue, currentValue) => previousValue + currentValue.PoisoningComplaint, 0),
        ConfirmedPoisoning: data.reduce((previousValue, currentValue) => previousValue + currentValue.ConfirmedPoisoning, 0),
        Brakerage: (data.reduce((previousValue, currentValue) => previousValue + currentValue.Brakerage, 0) / data.filter(item => item.Brakerage > 0).length).toFixed(2),
        SanPin: (data.reduce((previousValue, currentValue) => previousValue + currentValue.SanPin, 0) / data.filter(item => item.SanPin > 0).length).toFixed(2),
    });

    var data_source = data;
    let pos = 1;    

    $("#grid_quality").kendoGrid({
        toolbar: ["excel"],
        excel: {
            allPages: true,
        ...
        columns: [
            {
                ...
                headerAttributes: { style: "white-space: normal; vertical-align: middle; text-align: center;" },
                template: function (dataItem) { 
                    let isBold = dataItem.DepNum == 0 ? "font-weight: bold;" : "";
                    return '<font style="' + isBold + '">' + dataItem.DepName + '</font>';
                },
            },{
                field: "NegativeFeedbackCount",
                title: "Количество отрицательных отзывов",
                headerAttributes: { style: "white-space: normal; vertical-align: middle; text-align: center;" },
                template: function (dataItem) { 
                    let color = dataItem.DepNum != 0 && dataItem.NegativeFeedbackCount > 0 ? "red" : "";
                    let isBold = dataItem.DepNum == 0 ? "font-weight: bold;" : "";
                    return '<font color="' + color + '" style="' + isBold + '">' + dataItem.NegativeFeedbackCount + '</font>';
                },
                width: "150px",
                ...
                template: function (dataItem) { 
                    let isBold = dataItem.DepNum == 0 ? "font-weight: bold;" : "";
                    return '<font style="' + isBold + '">' + dataItem.StaffDismissed + '</font>';
                },
                attributes: { style: "text-align: right;"},
            },{
                field: "StaffTurnover",
                title: "Текучка в % (норма: не более 4%)",
                headerAttributes: { style: "white-space: normal; vertical-align: middle; text-align: center;" },
                template: function (dataItem) { 
                    let color = dataItem.DepNum != 0 && dataItem.StaffTurnover > 4 ? "red" : "";
                    let isBold = dataItem.DepNum == 0 ? "font-weight: bold;" : "";
                    return '<font color="' + color + '" style="' + isBold + '">' + dataItem.StaffTurnover + ' % </font>';
                },
                //format: GetFormat(item.FormatString),
                width: "150px",
                attributes: { style: "text-align: right;"},
            },{
                field: "PoisoningComplaint",
                title: "Жалоба на отравления",
                headerAttributes: { style: "white-space: normal; vertical-align: middle; text-align: center;" },                
                template: function (dataItem) { 
                    let color = dataItem.DepNum != 0 && dataItem.PoisoningComplaint > 0 ? "red" : "";
                    let isBold = dataItem.DepNum == 0 ? "font-weight: bold;" : "";
                    return '<font color="' + color + '" style="' + isBold + '">' + dataItem.PoisoningComplaint + '</font>';
                },
                width: "150px",
                attributes: { style: "text-align: right;"},
            },{
                field: "ConfirmedPoisoning",
                title: "Подтвержденное отравление",
                headerAttributes: { style: "white-space: normal; vertical-align: middle; text-align: center;" },
                template: function (dataItem) { 
                    let color = dataItem.DepNum != 0 && dataItem.ConfirmedPoisoning > 0 ? "red" : "";
                    let isBold = dataItem.DepNum == 0 ? "font-weight: bold;" : "";
                    return '<font color="' + color + '" style="' + isBold + '">' + dataItem.ConfirmedPoisoning + '</font>';
                },
                width: "150px",
                attributes: { style: "text-align: right;"},
            },{
                field: "Brakerage",
                title: "Бракераж в % (норматив: 90%)",
                headerAttributes: { style: "white-space: normal; vertical-align: middle; text-align: center;" },
                template: function (dataItem) { 
                    let color = dataItem.DepNum != 0 && (dataItem.Brakerage > 0 && dataItem.Brakerage < 90) ? "red" : "";
                    let isBold = dataItem.DepNum == 0 ? "font-weight: bold;" : "";
                    return '<font color="' + color + '" style="' + isBold + '">' + dataItem.Brakerage + ' % </font>';
                },
                width: "150px",
                attributes: { style: "text-align: right;"},
            },{
                field: "SanPin",
                title: "СанПин (норматив: 90)",
                headerAttributes: { style: "white-space: normal; vertical-align: middle; text-align: center;" },
                template: function (dataItem) { 
                    if (dataItem.DepNum == 0){
                        return '<font style="font-weight: bold;">' + dataItem.SanPin + '</font>';
                    }else{
                        let dep = dep_list.find(_dep => _dep.DepNum == dataItem.DepNum);
                        let depId = dep ? String(dep.DepId) : ""; 
                        let color = (dataItem.SanPin > 0 && dataItem.SanPin < 90) ? "red" : "";
                        let year = Number($("#quality_year").data("kendoDropDownList").value());
                        let month = Number($("#quality_month").data("kendoDropDownList").value());
                        ...
                        return '<a style="color:' + color + ';cursor:pointer;text-decoration: underline;" onclick="getSanPinDetails(' + depId + ', \'' + dateStart + '\', \'' + dateEnd + '\', 1);">' + dataItem.SanPin + '</a>';
                    }                   
                },                
                width: "150px",
                attributes: { style: "text-align: right;"},
        ...
        dataBound: function () {
            $('#grid_quality .k-grid-content').height("90%");
        },
        editable: false,
    });

    $("#grid_quality").data("kendoGrid").dataSource.read();
}