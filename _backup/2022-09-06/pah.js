...
function kengoObjectsInit(){
    ...
    $("#quality_kitchen_start").kendoDatePicker({
        ...
        close: function() {
            ...
            //var todayDate = kendo.toString(kendo.parseDate(this.value().setDate(1)), 'MM/dd/yyyy');
    ...
    //загрузка списков подразделений    
    $.ajax({
        ...
        success: function (data) {
            if (typeof (data) == "object") {
                dep_list = data;
                qualityDepartmentsList = data.map((item) => ({id: item.DepNum, name: item.DepName})).sort((a, b)=>a.id - b.id);
...
function PrepareQualityData(source, target) {
	...
	if ($('#grid_' + target + '>.k-grid-content-locked').css('height')){
		let newH = Number($('#grid_' + target + '>.k-grid-content-locked').css('height').replace("px","")) + 156;
		$('#grid_' + target + '>.k-grid-content-locked').css('height', newH + 'px');
	}
}
