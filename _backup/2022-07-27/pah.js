...
function kengoObjectsInit(){
    ...
    $("#quality_kitchen_month").kendoDropDownList({
        dataTextField: "title", 
        dataValueField: "value", 
    ...
    $("#quality_kitchen_month").data("kendoDropDownList").select(function(dataItem) {
        return dataItem.value == new Date().getMonth();
    });

    $("#quality_kitchen_year").kendoDropDownList(
        {placeholder: "", 
        clearButton: false,
        dataSource: yearsList,
    });
    $("#quality_kitchen_year").data("kendoDropDownList").select(function(dataItem) {
        return dataItem.toString() == new Date().getFullYear().toString();
    });
...
function RefreshQualityKitchen(){
    ...
    let monthDate = $("#quality_kitchen_year").data("kendoDropDownList").value() + $("#quality_kitchen_month").data("kendoDropDownList").value().padStart(2,"0") + "01";
    //console.log(monthDate);

    $.ajax({
        url: "https://" + host + "/complaints/api/info/GetQualityKitchenTable?MonthDate=" + monthDate,
        global: false,
        type: "GET",
        success: function (data) {
            console.log(data);
...
function RefreshQuality(){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (typeof (data) == "object") {
                PrepareQualityData(data, "quality")
                if (data.Message && data.Message != ""){
                    alert(data.Message);
                }
            };
...
function PrepareQualityData(source, target) {
    ...
    data.forEach(function (indicator) {
        data_source.forEach(function (item) {
            //item[indicator.Name] = null;
            item[indicator.Name] = 0;
        });
    });
    //заполняем таблицу
    data.forEach(function (indicator, index) {        
        Object.keys(indicator.Values).map(function(key) {
            data_source.forEach(function (value) {
                value.DepNum == key ? value[indicator.Name] = indicator.Values[key].Value : "";
            });
        });
    });
    ...
    data.forEach(function (indicator) {
        //console.log(indicator.SummaryType);
        if (indicator.SummaryType.includes("avg")){
            //console.log("avg");
            let sum = data_source.reduce((previousValue, currentValue) => previousValue + currentValue[indicator.Name], 0);
            let count = data_source.length;
            if (indicator.SummaryType.includes(">0")){
                count = data_source.filter(item => item[indicator.Name] > 0).length;             
            }
            summary[indicator.Name] = count > 0 ? Math.round(sum/count*100)/100 : 0;
        }
        if (indicator.SummaryType.includes("sum")){
            //console.log("sum");
            summary[indicator.Name] = data_source.reduce((previousValue, currentValue) => previousValue + currentValue[indicator.Name], 0);            
        }
        references[indicator.Name] = !(indicator.ReferenceCompare == "<=" && indicator.ReferenceValue == 0 ) && indicator.ReferenceCompare != "" ? indicator.ReferenceValue : "шт";
    });
    ...
    data.forEach(function (indicator) {
        ...
        if (indicator.Name == "SanPin"){
            template = function (dataItem) { 
                if (dataItem[indicator.Name] == null){
                    return "";
                }
                if (dataItem.DepNum <= 0){
                    return '<font style="' + isSummaryBold(dataItem) + '">' + getFormatedValue(dataItem[indicator.Name], indicator.Format, dataItem.DepNum) + '</font>';
                }else{
                    if (dataItem[indicator.Name] > 0){
                        ...
                        let year = Number($("#" + target + "_year").data("kendoDropDownList").value());
                        let month = Number($("#" + target + "_month").data("kendoDropDownList").value());
            ...
            template = function (dataItem) { 
                ...
                return '<font color="' + (dataItem.DepNum == 0 ? 'black' : getIndicatorColor(dataItem[indicator.Name], indicator.ReferenceValue, indicator.ReferenceCompare)) + '" style="' + isSummaryBold(dataItem) + "" + '">' + getFormatedValue(dataItem[indicator.Name], indicator.Format, dataItem.DepNum) + '</font>';
            };
        }
        columns.push({
    ...
    $("#grid_" + target).kendoGrid({
        ...
        dataBound: function () {
            $("#grid_" + target + " .k-grid-content").height("90%");
        },
        editable: false,
    });
