...
function PreparePaharData(data, filterDepartments, filterIndicators) {
    ...
    let data_source = [];
    let columns = [];
    let headerColumns = $("#pahar_cols").val();
    ...
    if (headerColumns == "indicators") {       
        ...
        data.forEach(function (item, idx) {
            ...
            column.title = item.Caption;
            column.format = GetFormat(item.FormatString);
    ...
    //таблица со столбцами по подразделения
        ...
        let departmentList = [];
        data.forEach(function (item, idx){
            Object.keys(item.Values).forEach(function(key, index){
                //добавляем фильтр по подразделению
                if (!filterDepartments || filterDepartments.length == 0 || filterDepartments.includes(GetCodeDepartmen(key))) {
                    if(!departmentList.some(i => i.DepId == key)){
                        if (GetCodeDepartmen(key)) {
                            departmentList.push({
                                DepId: key, 
                                DepCode: GetCodeDepartmen(key),
                                DepName: GetNameDepartmen(key)
                            });
                        }
                    }
                }
        ...
        departmentList.forEach((dep, idx) =>{
            ...
            column.template = function (item, excel = false, cell) {
                var value = item.FormatedValues[dep.DepId]; 
                if (!value)
                    return "";

                if (excel){
                    if(item.ReferenceFormatedValues)
                        if (item.ReferenceFormatedValues[dep.DepId])                    
                            value = value + " [" + item.ReferenceFormatedValues[dep.DepId] + "]"

                    if (item.ReferenceResult){
                        if (item.ReferenceResult[dep.DepId]) {
                            if (item.ReferenceResult[dep.DepId] == 1) 
                                cell.background = "#CCFFCC";
                            if (item.ReferenceResult[dep.DepId] == 2) 
                                cell.background = "#FFCCCC";
                            if (item.ReferenceResult[dep.DepId] == 3) 
                                cell.background = "#F1F1F1";
                        }
                    }
                } else {
                    if (item.TypeId == 0 && item.Caption != "Итого"){
                        value = "<a style='cursor:pointer;text-decoration: underline;' onclick='getSanPinDetails(" + String(dep.DepId) + ")'>" + value + "</a>"
                    }
                        
                    if (item.ReferenceResult){
                        if (item.ReferenceResult[dep.DepId]) {
                            switch (item.ReferenceResult[dep.DepId]) {
                                case 1:
                                    value = refSymDone + " " + value;
                                    break;
                                case 2:
                    ...
                    if(item.ReferenceFormatedValues)
                        if (item.ReferenceFormatedValues[dep.DepId])
                            value += " [" + item.ReferenceFormatedValues[dep.DepId].replace(" ", "") + "]";
                }
    ...
    function excelexport(e) {
        ...
        for (var i = 1; i < sheet.rows.length; i++) {
            ...
            data.data().forEach(function (itm) {
                //console.log(itm.field0)
                if (itm.field0 == row.cells[0].value || itm.Caption == row.cells[0].value)
                    item = itm;
            });
	...
	if ($('#grid_pahar>.k-grid-content-locked').css('height')){
        //let newH = Number($('#grid_' + target + '>.k-grid-content-locked').css('height').replace("px","")) + 17;
        //console.log($('#grid_pahar>.k-grid-content-locked').css('height'));
        $('#grid_pahar>.k-grid-content-locked').css('height', (550 - 17) + 'px');        
        //console.log($('#grid_pahar>.k-grid-content-locked').css('height'));
        //рамка таблицы
...
function kengoObjectsInit(){
    ...
    $("#pahar_start").kendoDatePicker({
        format: "dd/MM/yyyy",
        value: new Date(currDate.getFullYear(), currDate.getMonth()-1, currDate.getDate()),
        close: function() {
            //$("#quality_start").data("kendoDatePicker").value(kendo.toString(new Date((this.value()).getFullYear(), (this.value()).getMonth(), 1), 'dd/MM/yyyy'));            
        }
    });
...
//
function getColWidth(monthCount ) {
    switch(monthCount){
        case 1: return "180px";
        case 2: return "90px";
        //case 3: return "150px";
