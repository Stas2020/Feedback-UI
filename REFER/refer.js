                    ...
                    if (depStaff.Counts) {
                        depStaff.Counts.forEach(pos => {
                            if (Number(pos.posId) > 0) {
                                //кроме (148)уборщика ресторана, он группируется с посудомойщиком
                                if (pos.posId != 148) {
        ...
        if (mode == 1) {
            ...
            if (dataPos == null) {
                dataPos = {
                    ...
                    PosName: item.PositionId == 2 ? "Повар" : (item.PositionId == 14 ? "посудомойщик / уборщик ресторана" : item.PositionName),
                    StaffTable: GetStaffTableCount(staffTable, item.DepNum, item.PositionId),
                    StaffTableQuotes: GetStaffTableQuote(staffTable, item.DepNum, item.PositionId),
                    StaffCount: 0,//1,
                    StaffDismissed: (isDismissed ? 1 : 0)
...
function GetStaffTableQuote(staffTable, DepNum, PosId) {
    ...
    if (depRecs.length > 0) {
        ...
        depRecs.forEach(function (item) {
            var filterPos = [PosId];
            if (PosId == 14) { //группируем (14)посудомойщиков и (148)уборщиков ресторана
                filterPos.push(148);
            }
            var posRecs = (PosId != null && PosId != 0) ? item.Quotes.filter(_p => filterPos.includes(_p.posId)) : item.Quotes;
...
function GetStaffTableCount(staffTable, DepNum, PosId) {
    ...
    if (depRecs.length > 0) {
        ...
        depRecs.forEach(function (item) {
            ...
            var posRecs = (PosId != null && PosId != 0) ? item.Counts.filter(_p => filterPos.includes(_p.posId)) : item.Counts;
            posRecs.forEach(function (itm) {
                sum += itm.count;
            })
        })
...
function excelexportStaffTurnover(e) {
    ...
    data.data().forEach(function (dataItem) {
        ...
        row.cells[4].value = dataItem.StaffTable > 0 ? (String(Math.round(dataItem.StaffCount / (dataItem.StaffTable + dataItem.StaffTableQuotes) * 100)) + " %") : ""

        row.cells[7].value = dataItem.StaffCount > 0 ? (String(Math.round((dataItem.StaffDismissed / dataItem.StaffCount) * 100)) + " %") : ""

        row.cells[8].value = dataItem.StaffCount > 0 ?
            (String(Math.round(((dataItem.StaffDismissed / dataItem.StaffCount) * 100 * (365.0 / diffDays)))) + " %") : ""
